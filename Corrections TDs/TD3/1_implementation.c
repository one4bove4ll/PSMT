...
static pthread_cond_t cond_ressources = PTHREAD_COND_INITIALIZER;
...
void f_thread(unsigned int *ressources_necessaires)
{
    ...
    /* Boucle d'attente passive utilisant la condition */
    while (nb_ressources < *ressources_necessaires) {
        err = pthread_cond_wait(&cond_ressources, &mutex_ressources);
    ...
    /* On signale à ceux qui attendent que des ressources ont été libérées */
    err = pthread_cond_broadcast(&cond_ressources);
    ERROR_IF(err != 0, "Erreur pthread_cond_broadcast");
    err = pthread_mutex_unlock(&mutex_ressources);
    ERROR_IF(err != 0, "Erreur pthread_mutex_unlock");
}
